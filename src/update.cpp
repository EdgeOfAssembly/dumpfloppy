...
int update_files(analysis& a, const update_options& opt, std::ostream& err)
{
    ...
    if (a.trd.present)
    {
        err << "dumpfloppy: cannot update TRD images in this version\n";
        return -1;
    }
    if (a.apple.present)
    {
        err << "dumpfloppy: cannot update Apple DOS/ProDOS images in this version\n";
        return -1;
    }
    if (a.foreign.present)
    {
        err << "dumpfloppy: cannot update " << a.foreign.format
