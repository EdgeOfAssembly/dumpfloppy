...
**HEAD:** (see `git log -1`) · **version 0.24** · GitHub `EdgeOfAssembly/dumpfloppy`  
...
**Verify last green:** `make -s test` 185 cases / 3582 assertions (7 skipped /tmp PC copies); `make -s verify` CBMC SUCCESS (FAT12 + GCR).

## Next (pick one slice)

1. **Decode next:** WOZ DOS 3.3/ProDOS catalog, STX GEMDOS, IPF→ADF when tracks are standard. G71 if a fixture appears.
